...
class GetCategoryDetailsAction
{
    ...
    public function execute(Category $category): array
    {
        $productsQuery = Product::query()
            ->with(['inventory', 'primaryImage', 'vehicleModel.brand'])
            ->where('category_id', $category->id);

        $stats = $this->buildStats($category, (clone $productsQuery));
     ...
     */
    private function buildStats(Category $category, $productsQuery): array
    {
