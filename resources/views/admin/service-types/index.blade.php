    ...
    <div class="rounded-lg border border-border bg-surface overflow-hidden">
        <div class="px-5 py-4 border-b border-border">
            <p class="text-sm text-muted">
                <span class="text-text font-bold">{{ $serviceTypes->total() }}</span>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-border">
                        ...
                        <tr class="hover:bg-secondary/60 transition-colors">
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.service-types.edit', $serviceType) }}" class="inline-flex h-9 w-9 items-center justify-center rounded border border-sky-200 bg-sky-50 text-sky-700 hover:bg-sky-100 transition-colors" title="Editar">
                    ...
                    @empty
                        <tr><td colspan="4" class="px-5 py-12 text-center text-muted">No se encontraron servicios.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        @if ($serviceTypes->hasPages())
            <div class="px-5 py-4 border-t border-border">{{ $serviceTypes->links('vendor.pagination.admin') }}</div>
        @endif
    </div>
