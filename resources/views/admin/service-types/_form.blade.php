...
<div class="grid gap-5">
    ...
    <div>
        ...
        <textarea id="description" name="description" rows="4"
                  class="w-full rounded border border-border bg-surface px-4 py-2.5 text-sm text-text focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
                  placeholder="Describe el servicio para la tienda…">{{ old('description', $serviceType?->description) }}</textarea>
    </div>

    <div data-service-type-image>
        <label class="block text-xs font-bold uppercase tracking-wider text-muted mb-2">Imagen</label>

        @if ($serviceType?->image)
            <div class="mb-3 flex items-center gap-3 rounded border border-border bg-secondary p-3">
                <img src="{{ $serviceType->image }}" alt="" class="h-16 w-16 rounded object-cover border border-border">
...
<script>
    (function () {
        const root = document.querySelector('[data-service-type-image]');
        if (!root) return;
